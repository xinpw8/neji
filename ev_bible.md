...
Note:     Nova's resources all start at ID number 128, but the internal storage
for all data file info is zero-based. Therefore, when a field in the Nova data
file is said to refer to a government, stellar object, etc., it refers to it by
its index number (starts at 0) unless it is specifically stated that it is
referring to the ID number, which starts at 128.

Note:     Some of Nova's fields refer to other resource IDs or index numbers,
but their values are offset by a certain amount to indicate type. For example,
the m•sn resource's AvailStel field refers to the index number of a gšvt
resource when its value is between 10000 and 10255. In cases like this, it is
necessary to add to or subtract from the field in order to force the value into
the proper range: in this instance, you'd subtract 10000 to find the index
number of the gšvt.

Note:     Any resources in an Nova plugin file automatically replace
same-numbered resources in NovaÕs main files. Resources are loaded from the
"Nova Files" folder first and then resources from the "Nova Plug-Ins" folder are
loaded.
...
loaded from sp•n resources 400-463, the sounds are loaded from snd resources
300-363, and the behavior of each explosion type is contained within bššm
resources 128-191. Each bššm resource contains three fields:
...
SoundIndex     The index (0-63) of the explosion sound to associate with this
                ...
                itself (e.g. 1 for bššm resource 129, etc.) but if you want to
...
GraphicIndex   The index (0-63) of the explosion graphic to associate with this
                ...
                explosion index itself (e.g. 1 for bššm resource 129, etc.) but
...
* The chŠr resource
	...
	The chŠr resource is used to allow multiple entry points into the scenario's
storyline, by letting the player pick a "character template" from a list when a
new pilot is started. The player is presented with a list of all available chŠr
...
IntroTextID   The ID of the d‘sc resource to show when the player starts out
               ...
               for example). Using the d‘sc resource's MovieFile field here
...
Flags         0x0001    Denotes the "default" chŠr resource, which will be
                         automatically selected in the popup menu. If more
                         than one chŠr resource has this bit set (there
...
* The cšlr resource
	...
	The cšlr resource allows you to customize some game-wide interface
options.
...
Note that all color fields in the cšlr resource are encoded the same as
HTML colors, and that only the first cšlr resource is loaded.
...
If you wish, you can make a d‘sc resource mutable via control bits - embedding a
special sequence of characters into the d‘sc resource will instruct Nova to
...
compound tests in a d‘sc resource - i.e., no testing of multiple bits at a time.
...
For example, consider this d‘sc resource:
...
Besides the Description field in the d‘sc resource, there are some additional
fields:
...
VoiceType          Sets this government's voice type, used for when you have
                   ...
                   with no inherent attributes govt defined (see sh•p section
...
InitialRec         The playerÕs initial legal record in systems controlled by
...
MaxOdds            The maximum combat odds ships of this govt will consider
                     ...
                     is taken from the Strength field in the sh•p resource,
...
Interface          ID of an •ntf resource to use when the player is flying a
...
0x0040     Never attacks player (also, playerÕs weapons canÕt hit them)
...
0x0100     ÔpersÕ ships of this govt wonÕt use escape pod, but will act as if
             they did
...
ScanMask field in the m•sn resource. If any of the 1 bits in a government's
...
ScanMask   If this is an "illegal" cargo type, this is used in conjunction with
            the ScanMask field in the gšvt resource. If any of the 1 bits in a
            ship's government's ScanMask field match any of the 1 bits in a
            jŸnk type's ScanMask field, that government will consider that
...
Abbrev     The short string that is displayed in the player's status bar when
            the player is carrying jŸnk of this type, e.g. "Parts".
            
BuyOn      Control bit test expression. This jŸnk will only be available to be
            bought when this expression evaluates true. Leave blank if unused.
            
SellOn     Control bit test expression. This jŸnk will only be able to be
...
* The •ntf resource
	...
	The •ntf resource controls the appearance of the status bar by modifying
the position and color of the various status bar elements, as well as changing
the status bar background image.  Additionally, the use of multiple •ntf
resources, along with proper values in the various gšvt resources' Interface
...
Note that all colors in the •ntf resource are encoded the same as HTML colors,
...
AvailLoc        Where on a planet this mission is available
                ...
                1            In the bar
                2            Offered from ship (must set up associated p‘rs
                               resource as well)
...
PayVal          What you get if you're successful and you return to ReturnStel
                ...
                -40001 to -40099   Take away this % of the playerÕs cash
                                     (-40001 = 1%, -40002 = 2%, etc.)
...
ShipStart       Defines where in the system the ships will start
                ...
                is the proper value to use in conjunction with p‘rs
...
Flags           Some misc. flag bits
                0x0001         Marks the mission as an auto-aborting mission,
                                 ...
                                 the missionÕs CompBitSet fields will be
                                 ...
                                 replaces a p‘rs ship at mission start (such
                ...
                0x0002         DonÕt show the red destination arrows on the map
                0x0004         CanÕt refuse the mission
                ...
                0x2000         Mission unavailable if playerÕs ship is of
                                 inherentAI type 1 or 2  (cargo ships)
                0x4000         Mission unavailable if playerÕs ship is of
...
* The n‘bu resource
     ...
     N‘bu resources contain info on the nebulae (or other space phenomena) which
are displayed in the background of the star map. These images donÕt actually
have any effect on events in the game, theyÕre just there to look pretty. You
can, however, combine n‘bu background images with custom asteroid or
interference data in the sØst resources for cool localized effects, and you can
also use the n‘bu resource's OnExplore field to enable events when the player
explores a certain nebula.

The PICT resources associated with the 32 available n‘bu resources are numbered
...
The effects of the n‘bu resourceÕs fields are as follows:

XPos     The imageÕs position on the star map. These coordinates are
YPos       expressed in the scale of the ÒnormalÓ map zoom level (not zoomed
           in or out) and are relative to the upper-left corner of the image.

XSize    The imageÕs size on the star map. These values are expressed in
YSize      the scale of the ÒnormalÓ map zoom level (not zoomed in or out) and
...
ÒdisastersÓ is a misnomer, as these occurrences simply affect the price of a
...
choose ÒOutfit ShipÓ at a planet or station.
...
TechLevel          What the technology level of the item is. This item will be
                     ...
                     SpecialTech fields of the spšb resource; see the section
                     on spšb resources for more information.)

The next two fields tell Nova what kind of modification this item performs:

If ModType is: Then itÕs:              And ModVal refers to:
1              a weapon                The ID number of the associated w‘ap
                                         resource
2              more cargo space        The number of tons of cargo space to
                                         add
3              ammunition              The ID number of the associated w‘ap
                                         resource
...
7              acceleration booster    Amount of accel to add (see sh•p for
                                         more info)
8              speed increase          Amount of speed to add (see sh•p for
                                         more info)
...
18             fuel scoop              How many frames per 1 unit of fuel
                                         ...
                                         perform the same function in Òfuel
                                         suckingÓ mode
...
22             hyperspace speed mod    Number of days to increase or decrease
                                         shipÕs hyperspace travel time (still
                                         canÕt go below 1 day/jump)
23             hyperspace dist mod     Amount to increase or decrease the
                                         no-jump zoneÕs radius by (the
                                         standard radius is 1000) 
24             interference mod        Subtracts the value in ModVal from the
                                         current star systemÕs Interference
                                         value when calculating how ÒfuzzyÓ to
                                         make the radar scanner
25             marines                 Adds the value in ModVal to your shipÕs
...
50             nonlethal bomb           randomly destroys itself and damages
                                         ...
                                         set this field to the ID of a bššm
...
Flags     Miscellaneous info:
          ...
          0x0008     This item canÕt be sold
...
ÒalternateÓ functions for an outfit item - e.g., a weapon could also reduce the
shipÕs turning speed.  The only restriction on ModType2-4 is that you shouldnÕt
...
ItemClass       The item's classification, used in the p‘rs resource for
                 items that are given out by non-player characters' ships.
                 Set to 0 or -1 if unused.

ScanMask        If this is an "illegal" outfit type, this is used in
                 conjunction with the ScanMask field in the gšvt resource. If
                 any of the 1 bits in a ship's government's ScanMask field
                 match any of the 1 bits in an oŸtf type's ScanMask field, that
...
their names (which are also the names of the associated p‘rs resource) displayed
...
AI Type        The person's AI type (see the section on dŸde resources)
               1               Wimpy trader
               2               Brave trader
               3               Warship
               4               Interceptor
...
Flags             Some control bits
     ...
     0x0040       When LinkMission is accepted with a single SpecialShip,
                   ...
                   missions. Note: if the mission's SpecialShip dŸde type
                   contains the p‘rs ship's ship type in it, the SpecialShip
                   that's created will be of the same type as the p‘rs ship,
                   regardless of the probabilities in the dŸde resource. This
                   is to prevent a p‘rs ship from accidentally morphing into
                   another ship type before the player's eyes. If you really do
                   want to have the p‘rs ship be replaced by a SpecialShip of
                   a different type, use a dŸde in the mission's ShipDude
                   field that doesn't contain the p‘rs's ship type.
     0x0080       Only show quote once
     0x0100       Deactivate ship (i.e. donÕt make it show up again) after
                    accepting its LinkMission
     0x0200       Offer shipÕs LinkMission when boarding it instead of when
                    hailing it
     0x0400       DonÕt show quote when shipÕs LinkMission is not available
     0x0800       Make ship leave after accepting its LinkMission
     0x1000       DonÕt offer if player is flying a wimpy freighter (aiType 1)
     0x2000       DonÕt offer if player is flying a beefy freighter (aiType 2)
     0x4000       DonÕt offer if player is flying a warship (aiType 3)
...
The name of the rŠnk resource is the full name of the rank, displayed in the
player-info dialog.
example: "Commission of Space Marshall in the Hector Empire"

The text fields in the rŠnk resource are:
...
properties. ršid resources 128-143 store the attributes for each asteroid type:
...
YieldType    Defines what the resource-boxes which are ejected from this
               ...
               1000-1127   This type of jŸnk resource
...
ExplodeType  Type of explosion to show (0-63) when an asteroid of this type
               ...
               the same manner as the ExplodeType field in the w‘ap resource.
...
FreeMass          Space available to add additional items and upgrades. Note
                    ...
                    shipÕs stock weapons. (e.g. a ship with 20 tons listed in
...
The next two fields tell Nova what this shipÕs maximum loadout of fixed guns and
...
MaxGun     The shipÕs maximum number of fixed guns, which are flagged in the
             WeapFlag field of the outf resource.

MaxTur     The shipÕs maximum number of turrets, which are flagged in the
             WeapFlag field of the outf resource.
...
TechLevel     What the technology level of the ship is. This ship will be
                ...
                fields of the spšb resource; see the section on spšb resources
...
Length     The ship's length in meters. Currently unused in any calculations,
             but itÕs kinda cool, so itÕs displayed in the "detailed ship info"
             dialog.
...
voice type, distress message flags, etc, as well as for inherent jamming
ability). Sometimes you might want to create a ship type that inherits
attributes from a particular govt but isn't considered to be inherently
of that govt in combat, so Nova lets you use the InherentGovt field in
...
Flags        0x0001     Slow jumping (75% normal speed)
             ...
             0x0100     Show % armor on target display instead of ÒShields
                          DownÓ
             0x0200     Don't show armor or shield state on status display
             0x0400     Ship is a planet-type ship, and can only be hit by
                          planet-type weapons
             0x1000     ShipÕs turrets have a blind spot to the front
             0x2000     ShipÕs turrets have a blind spot to the sides
             0x4000     ShipÕs turrets have a blind spot to the rear
...
PodCount     For decorative purposes, AI ships can be made to launch escape
             ...
             to do with the p‘rs field's escape pod flag, it's just for eye
...
DefaultItems   Up to eight default items with which to equip this ship when the
                 ...
                 will ignore these fields; also, donÕt put anything in here
                 that isnÕt a physical item - i.e. afterburners, shield
...
FuelRegen      This ship typeÕs inherent fuel regeneration property. Works
...
SkillVar       The amount (in percent) to which this shipÕs pilotsÕ skill
                 varies. This affects acceleration and turn rate for each ship:
                 i.e. a skill variance of 10% would make each ship of a given
                 type up to 10% slower or faster than ÒstockÓ. Values from
                 1 to 50% are valid.
...
HyperLink1-8    IDs of the spšb resources of up to eight other hypergates or
...
NavDef (x16)         Navigation defaults (F1-F4)
                    -1           No nav default for this key
                     128-548     ID number of the stellar object to set as a
                                   default
...
% Prob (x8)             Probability that a given ship placed is of each of
                          the above types
...
Want to make a ÔpersÕ type ship always appear? Put its ID into one of the Person
...
Note: Using the Visibility field to replace one system with another will work
...
AstTypes       Flag bits that determine what types of asteroids will appear
               in this system:
               0x0001    Small metal   (ršid ID 128)
               0x0002    Medium metal  (ršid ID 129)
               0x0004    Large metal   (ršid ID 130)
               0x0008    Huge metal    (ršid ID 131)
               0x0010    Small ice     (ršid ID 132)
               0x0020    Medium ice    (ršid ID 133)
               etc.
...
Guidance      The weapon's guidance mode
              -1      Unguided projectile
               ...
               7      Front-quadrant turret, (can fire +/-45¡ off the ship's
...
Graphic        What graphic set to use for this weapon
               0-255             Use this graphic set (i.e. spin resources
                                  3000-3255)
...
Flags          0x0001           Spin the weapon's graphic continuously (rate
               ...
               0x0800           WeaponÕs smoke trail is more persistent
...
SmokeSet       Which cicn set to use for this weaponÕs smoke trail, if any.
               0 = cicnÕs 1000-1007, 1 = 1008-1015, etc.  Note that the smoke
               icons themselves can be any size, but if you use ResEditÕs cicn
               editor to make each icon only as large as it needs to be, game
               performance will likely improve.

Decay          How fast to decay each shotÕs power
               -1 or 0          Ignored
...
MaxAmmo        For ammo-using weapons, this is the maximum amount of ammo
                ...
                want the ammo quantity to be constrained by the oŸtf
...
Another Note:  If you don't create an oŸtf resource for each weapon type, your
...
create oŸtf resources)  Also, having multiple outfit items that bestow the
...
The solution is not to change the STR# resources at all, but to use NovaÕs handy
...
change the first cargo type from food to something else, youÕd simply create a
...
Govt-specific greetings for comm dialog           10000-12559 (first 10 for
                                                    ...
                                                    1, etc.)
p‘rs-resource CommQuotes                          15000-15511
